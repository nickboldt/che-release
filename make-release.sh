...
bump_version() {
  ...
  if [[ $UPDATE_PARENT -eq 1 ]]; then
    echo "Bumping parent to ${NEXTVERSION}"
...
if [[ $TRIGGER_RELEASE -eq 1 ]]; then
  ...
  mvn versions:update-parent versions:commit -DparentVersion=${VERSION}
  git commit -a -s -m "Set release version of parent"
  echo "Parent updated in release branch"
...
if [[ "${BASEBRANCH}" != "${BRANCH}" ]]; then
  ...
  echo "bumping version in basebranch: ${BASEBRANCH}"
  bump_version ${NEXTVERSION_Y} ${BASEBRANCH} 1
fi
# bump the z digit
...
echo "bumping version in ${BRANCH}"
